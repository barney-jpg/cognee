...
async def get_retriever_output(
    ...
    # graph_retrieval stage: object retrieval + context extraction. Not emitted on the early
    # session-turn answer path (that returns above, before this point).
    try:
        ...
        _attach_stage(exc, "graph_retrieval", dataset_name)
        raise

    await emit("graph_retrieval", "completed", _stage_detail(dataset_name, object_count=obj_count))

    completion = None
    if not only_context:  # If only_context is True, skip completion. Performance optimization.
        # llm_generation stage: started only (the LLM call is the long, silent phase keepalives
        # protect); skipped entirely when only_context=True or on the early-answer path above.
        await emit("llm_generation", "started", _stage_detail(dataset_name))
        ...
        try:
            ...
            _attach_stage(exc, "llm_generation", dataset_name)
            raise
